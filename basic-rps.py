...
class Text(Enum):
    ...
    """
    PLAY = "Hello, do you want to play a game of rock, paper, scissors? Please answer when my eyes turn green, then I am listening."
...
def clean_context():
    return {
        ...
        'robot_choice': None,
        # whether given input from user is wrong/not understood
        "invalid_user_input": False,
        # wins, ties, loses (from a robot's perspective)
        "result": {Result.WON: 0, Result.TIE: 0, Result.LOST: 0},
        # whether a reset request has been made
        "reset": False
    }


class BasicRockPaperScissors:
    ...
    def on_left_foot_pressed(self) -> None:
        """
        Resets the context.
        """
    ...
    def run(self) -> None:
        ...
        Run the RPS game
        """
        self.sic.start()  # starts the main thread
        # activate the foot listeners (one to start and other to reset context)
        ...
        # play procedure
        def _play():
            # reset context if left foot was pressed
            ...
            if self.context['wanna_play']:
                ...
                # user's choice retrieval procedure
                ...
                # Choose some random action
                self.context['robot_choice'] = random.choice([RPS.ROCK, RPS.PAPER, RPS.SCISSORS])
                self.say(Text.I_CHOOSE, {"{choice}": self.context['robot_choice'].value})
                ...
                # The user does not want to play anymore, stopping the SIC and telling the scores
                self.say(Text.OKAY)
                ...
                # Dump the result (for scientific goals)
                with open(f'results/basic-{datetime.timestamp(datetime.now())}-results.txt', mode='w') as f:
    ...
    def yes_no_qa(self, ass_var: str, result) -> None:
        """
        ...
        print(result)
        # if not understood, invalidate the result
        if result is None or (not ('yes' in result['text'].lower())
                              and not ('no' in result['text'].lower())):
    ...
    def rps_qa(self, ass_var: str, result):
        ...
        print(result)
        # if not understood, invalidate the result
        if result is None or (not ('rock' in result['text'].lower())
                              and not ('paper' in result['text'].lower())
                              and not ('scissors' in result['text'].lower())):
            self.context['invalid_user_input'] = True
            return
        self.context['invalid_user_input'] = False
...
if __name__ == '__main__':
    rps = BasicRockPaperScissors('127.0.0.1')
    rps.run()
